...
def recommend(index, method):
    ...
    # Return top 3 most similar articles (exclude self)
    similarity_scores = similarity_scores[1:4]
    print(similarity_scores)
    
    articles_index = [i[0] for i in similarity_scores]
    
    # Return the title of the most similar articles
...
# Pre-process text: convert cases, lemmatize, remove stopwords(general & Python specific)
for i in range(0,len(articles_all)):
    if np.remainder(i,250) == 0:
        print("Now lemmatizing article {}".format(i))
    articles_all["text"].iloc[i] = lemmatize_text(articles_all["text"].iloc[i])
...
# Pickle pandas frame
articles_all.to_pickle('../draftingboard/draftingboard/articles_all_cosine')